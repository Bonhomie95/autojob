...
def store_cv(user_id: str, filename: str, data: bytes, max_bytes: int) -> CvDocument:
    ...
    for doc in db.session.scalars(
        db.select(CvDocument).where(CvDocument.user_id == user_id)
    ):
        doc.is_active = False

    existing = db.session.scalar(
        db.select(CvDocument).where(
            CvDocument.user_id == user_id, CvDocument.content_hash == digest
        )
    )
    if existing:
...
def active_cv(user_id: str) -> CvDocument | None:
    return db.session.scalar(
        db.select(CvDocument).where(
            CvDocument.user_id == user_id, CvDocument.is_active.is_(True)
        )
    )
