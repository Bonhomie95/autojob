        ...
        if match:
            ...
            profile[field] = next(
                (
                    v
                    for v in found
                    if _normalise_for_compare(field, v) == _normalise_for_compare(field, env_value)
                ),
                env_value,
            )
        else:
            ...
            try:
                if pattern is _RANGE_PATTERNS[0]:
                    ...
                    start = _abs_month(int(groups[1]), int(groups[0]))
                        ...
                        _abs_month(int(groups[3]), int(groups[2]))
                        if groups[2] and groups[3]
                        else now
                    )
                else:
...
def _looks_like_title(line: str) -> bool:
    ...
    return any(w in vocab.TITLE_TOKENS for w in words) or bool(vocab.detect_seniority(bare))
...
def _parse_experience(section: str) -> list[dict]:
    ...
    for line in lines:
        ...
        if ranges and not bullet:
            ...
            parts = [
                p.strip(" ,|·—-") for p in re.split(r"[|·]|\s{2,}", remainder) if p.strip(" ,|·—-")
            ]
...
def _parse_undated_experience(section: str) -> list[dict]:
    ...
    for line in section.split("\n"):
        ...
        if _looks_like_title(bare):
            if current:
                entries.append(current)
            parts = [p.strip(" ,|·—-") for p in re.split(r"[|·]|\s{2,}", bare) if p.strip(" ,|·—-")]
            title, company = bare, ""
            if len(parts) > 1:
...
def _parse_projects(section: str) -> list[dict]:
    ...
    for line in section.split("\n"):
        ...
        is_header = not bullet and (numbered or (len(bare) <= 70 and not bare.endswith(".")))
...
def _parse_education(section: str) -> list[dict]:
    ...
    for line in section.split("\n"):
        ...
        parts = [
            p.strip() for p in re.split(r"[|·]|\s{3,}", bare) if p.strip() and p.strip() != year
        ]
        if len(parts) > 1:
            flush()
            entries.append(
                {
                    "degree": parts[0],
                    "school": next((p for p in parts[1:] if _SCHOOL_RE.search(p)), parts[1]),
                    "year": year,
                }
            )
            continue
...
def _derive_titles(experience: list[dict], summary: str) -> list[str]:
    ...
    def add(raw: str):
        cleaned = re.sub(
            r"\b(senior|sr|junior|jr|lead|staff|principal|intern)\b", "", raw, flags=re.I
        )
...
def _derive_seniority(years: float, experience: list[dict]) -> str:
    ...
    for entry in experience:
        ...
        if level and (
            not titled or vocab.SENIORITY_ORDER.index(level) > vocab.SENIORITY_ORDER.index(titled)
        ):
            titled = level
...
def validate_profile(profile: dict) -> list[str]:
    ...
    for field in profile.get("ambiguous_fields", []):
        options = profile.get("contact_choices", {}).get(field, {}).get("options", [])
        detail = (
            f"CV gives {len(options)} values for {field} "
