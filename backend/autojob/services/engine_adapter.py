...
_MANAGED_ATTRS = (
    "AI_PROVIDER", "GROQ_API_KEYS", "OPENAI_API_KEYS", "ANTHROPIC_API_KEYS",
    "GEMINI_API_KEYS", "GROK_API_KEYS", "OPENROUTER_API_KEYS",
...
# One of these (matching cfg.ai_provider) gets this run's ai_keys; the rest
# are cleared so a previous run's keys for a different provider can't leak
# into this one (see the save/restore in use_runtime below).
_AI_KEY_ATTRS = {
    "groq": "GROQ_API_KEYS",
...
def use_runtime(cfg: RuntimeConfig, profile: dict):
    ...
    with _engine_lock:
        ...
        try:
            legacy_config.AI_PROVIDER = cfg.ai_provider
            for provider, attr in _AI_KEY_ATTRS.items():
                setattr(legacy_config, attr, list(cfg.ai_keys) if provider == cfg.ai_provider else [])
