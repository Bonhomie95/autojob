...
#
# A run nobody ever subscribes to (e.g. Celery/scheduler-dispatched, with no
# live browser tab watching) has no consumer to ever call _drop_local_queue —
# every publish() would otherwise accumulate into a queue that sits in
# _local_bus forever. _MAX_QUEUE_AGE bounds that: a queue older than this with
# no consumer camped on it gets swept on the next bus access, well past any
# realistic run duration.
_local_bus: dict[str, tuple[queue.Queue, float]] = {}
_local_lock = threading.Lock()
_MAX_QUEUE_AGE = 1800.0
...
def _local_queue(run_id: str) -> queue.Queue:
    with _local_lock:
        _sweep_stale_locked()
        entry = _local_bus.get(run_id)
        if entry is None:
            q = queue.Queue()
            _local_bus[run_id] = (q, time.monotonic())
            return q
        return entry[0]


def _sweep_stale_locked() -> None:
    """Drop queues older than _MAX_QUEUE_AGE. Caller must hold _local_lock."""
    now = time.monotonic()
    stale = [rid for rid, (_, created) in _local_bus.items() if now - created > _MAX_QUEUE_AGE]
    for rid in stale:
        del _local_bus[rid]
