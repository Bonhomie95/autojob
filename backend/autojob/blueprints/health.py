...
@limiter.exempt
def metrics():
    # Unauthenticated by default (fine on an internal network); set
    # METRICS_TOKEN to require a matching header on a public deploy.
    token = current_app.config.get("METRICS_TOKEN", "")
    if token and not hmac.compare_digest(request.headers.get("X-Metrics-Token", ""), token):
