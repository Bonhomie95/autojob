...
class Config:
    ...
    def _load(self):
        ...
        # Groq is the default (generous free tier, fast). Also supported:
        # openai, anthropic, gemini, openrouter — see core/groq_client.py for
        # the dispatch and .env.example / README.md for where to get each key.
        self.AI_PROVIDER: str = self._get("AI_PROVIDER", "groq").strip().lower()
        # Blank = each provider's own sensible default model (see groq_client.py).
        self.AI_MODEL: str = self._get("AI_MODEL", "").strip()

        # ── Groq
        self.GROQ_API_KEYS: list[str] = self._load_groq_keys()
        self.GROQ_MODEL: str = "llama-3.3-70b-versatile"

        # ── Other AI providers (single key or a pool, same _1/_2/... pattern
        # as Groq/Hunter above).
        self.OPENAI_API_KEYS: list[str] = self._load_key_pool("OPENAI_API_KEY")
        self.ANTHROPIC_API_KEYS: list[str] = self._load_key_pool("ANTHROPIC_API_KEY")
        self.GEMINI_API_KEYS: list[str] = self._load_key_pool("GEMINI_API_KEY")
        self.GROK_API_KEYS: list[str] = self._load_key_pool("GROK_API_KEY")
        self.OPENROUTER_API_KEYS: list[str] = self._load_key_pool("OPENROUTER_API_KEY")
